...
description: Full-lifecycle NFT collection builder for Pharos — three artwork modes (generative with trait layers and rarity weights composed off-chain via Pillow, individual images, or single image), IPFS upload via Pinata, ERC-721 smart contract deployment via Foundry with 6 mint types (public free/paid, owner-only, whitelist free/paid, whitelist free + public paid), contract verification, optional minting landing page, batch minting, airdrop, Merkle tree whitelist, holder analytics with export, local state persistence, image validation, and auto dependency checking.
version: 2.0.0
frameworks:
  - openclaw
  - claude-code
